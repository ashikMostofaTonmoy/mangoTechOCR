...
class ImageViewer(QtWidgets.QGraphicsView):
    ...
    # def paintEvent(self, obj, event):
    #     painter = QtGui.QPainter(self)
    #     painter.drawPixmap(QtCore.QPoint(), self._pixmap_item)
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == QtCore.QEvent.GraphicsSceneMouseMove:
            ...
            self.destination = self._pixmap_item.mapFromScene(
                event.scenePos()).toPoint()
            # self.destination = event.scenePos()
            self.update()

        # t ended line before this
